...
export default function StudentLogin() {
    ...
    const handleLogin = async(e) => {
        e.preventDefault()
        const response = await axios.post('https://sprittle-assignment-server0.vercel.app/api/studentlogin', {
            email:email,
            password:password
    })
  ...
  return (
    <div className='MasterLoginCon'>
        ...
        <p className='registerquo'>Dont have register yet? <span className='registerflow'><Link to='/StudentRegister'>Register now</Link></span></p>
        </div>
    </div>
