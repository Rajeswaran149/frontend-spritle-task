...
export default function Question(props) {
  ...
  const sentParameters = () => {
    const response = axios.post(
      "https://backend-spritle-task.vercel.app/api/studentinputs",
      {
    ...
    axios.post("https://backend-spritle-task.vercel.app/api/Questiondelete", {
      id: props.id,
    });
      ...
      <div className="queinner">
        ...
        <div className="inputs">
          <input
            className="input"
            onChange={(e) => {
              setParamter1(e.target.value);
            }}
            placeholder="Enter the 1st parameter"
          />
          <input
            className="input"
            onChange={(e) => {
              setParamter2(e.target.value);
            }}
            placeholder="Enter the 2nd parameter"
          />
          <input
            className="input"
            onChange={(e) => {
          ...
          />
          <button className="send" onClick={sentParameters}>
            Send parameters <GrSend />
          </button>
        </div>
      </div>
    </div>
